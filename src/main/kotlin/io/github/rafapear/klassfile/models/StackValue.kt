...
/**
 * Represents a simulated operand-stack value tracked by [io.github.rafapear.klassfile.builders.CodeScope].
 *
...
sealed class StackValue(open val type: KlassDesc<*>) {
    ...
    fun withType(type: KlassDesc<*>): StackValue {
        return when (this) {
            ...
            is Parameter -> Parameter(ref.withType(type))
            is Local -> Local(ref.withType(type))
            is Field -> Field(ref.withType(type))
            is ReturnValue -> ReturnValue(ref.withType(type))
