...
private fun <O : Any, R : Any> filterMethods(
    name: String,
    owner: KlassDesc<O>,
    type: KlassDesc<R>,
    ...
    val methods: List<MethodRef<O, R>> = (crudeMethods + crudeConstructors).filter {
...
fun <O : Any, R : Any> resolveMethod(
    ...
    val filtered = filterMethods(name, owner, type, *params)

    // For error reporting, we need to construct the MethodRef for the requested method signature
 ...
 * @throws IllegalStateException if multiple overloads remain after filtering.
 */
fun <O : Any, R : Any> resolveMethodOrNull(
    name: String,
    owner: KlassDesc<O>,
    type: KlassDesc<R>,
    vararg params: ParamRef<*>,
