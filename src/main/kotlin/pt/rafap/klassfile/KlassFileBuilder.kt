...
import pt.rafap.klassfile.builders.FieldBuilder
...
import pt.rafap.klassfile.utils.KlassFileDsl
...
class KlassFileBuilder private constructor(val name: String, builder: KlassFileBuilder.() -> Unit = {}) {
	...
	private var flags: Int = ACC_PUBLIC
	private val fields = mutableListOf<ClassBuilder.() -> Unit>()
	...
	private val fieldBuilder = FieldBuilder(thisClassDesc())
	...
	/**
	 * Returns the [ClassDesc] that represents the class currently being built.
	 *
	 * @return the [ClassDesc] for the generated class.
	 */
	fun thisClassDesc(): ClassDesc = classDesc(name)

	/**
	 * Sets the access flags for the class being built.
	 *
	 * This value replaces any previously configured flags.
	 *
	 * @param value the bitwise combination of class access flags such as [ACC_PUBLIC], [ACC_FINAL], or
	 * [ACC_ABSTRACT].
	 */
	fun flags(value: Int) {
		flags = value
	...
	fun getFieldBuilder(): FieldBuilder = fieldBuilder

	fun fields(builder: FieldBuilder.() -> Unit) {
		getFieldBuilder().apply(builder)
	}

	inline fun <reified T : Any> field() = EagerDelegate<FieldRef<T>> { _, property ->
		val name = property.name
		field(name, classDesc<T>(), 0)
	}
	...
	inline fun <reified T> method(
		...
		return EagerDelegate { _, property ->
			...
			MethodRef(
				owner = thisClassDesc(),
				name = name,
				methodTypeDesc = MethodTypeDesc.of(classDesc<T>(), *params.toTypedArray()),
				flags = flags,
				isConstructor = false
			).also {
				method(it, body)
	...
	inline fun <reified T : Any> inherit() {
		...
		else throw IllegalArgumentException("Class ${clazz.qualifiedName} is neither an interface nor an abstract class, cannot be implemented or extended")
	}
	...
	fun CodeBuilder.invokevirtual(methodRef: MethodRef): CodeBuilder {
		if (methodRef.owner == thisClassDesc()) aload(0)
	...
	fun CodeBuilder.invokeMethod(methodRef: MethodRef) {
		val isStatic = methodRef.flags and ACC_STATIC != 0
		val isSpecial = methodRef.isConstructor

		when {
			isStatic -> invokestatic(methodRef)
			isSpecial -> invokespecial(methodRef)
			else -> {
				if (methodRef.owner == thisClassDesc()) aload(0)
	 ...
	 * @return this [CodeBuilder] for chaining.
	 */
	fun CodeBuilder.getfield(ref: FieldRef<*>): CodeBuilder = getfield(toFieldRefEntry(ref))
	 ...
	 * @return this [CodeBuilder] for chaining.
	 */
	fun CodeBuilder.putfield(ref: FieldRef<*>): CodeBuilder = putfield(toFieldRefEntry(ref))
	...
	inner class Klass {
		 ...
		 */
		inline fun <reified T : Any> writeAndGetInstance(): T = writeAndGetClass<T>().createInstance()
	...
	private fun build(): ByteArray {
		...
		return ClassFile.of().build(ClassDesc.of(name)) { clb ->
			...
			(fields + fieldBuilder.fields).forEach { clb.it() }
